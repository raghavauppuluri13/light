# https://github.com/real-stanford/diffusion_policy/blob/548a52bbb105518058e27bf34dcf90bf6f73681a/diffusion_policy/common/replay_buffer.py
...
def rechunk_recompress_array(group,
                             name,
                             chunks=None,
                             chunk_length=None,
                             compressor=None,
    ...
    n_copied, n_skipped, n_bytes_copied = zarr.copy(source=old_arr,
                                                    dest=group,
                                                    name=name,
                                                    chunks=chunks,
...
class ReplayBuffer:
    ...
    def __init__(self, root: Union[zarr.Group, Dict[str, dict]]):
        ...
        for key, value in root["data"].items():
            print(key)
            print(root["meta"]["episode_ends"][-1])
            print(value.shape[0])
    ...
    def save_to_path(
        ...
        **kwargs,
        ...
        return self.save_to_store(store,
                                  chunks=chunks,
                                  compressors=compressors,
                                  if_exists=if_exists,
                                  **kwargs)
    ...
    @classmethod
    def _resolve_array_compressor(cls, compressors: Union[dict, str, numcodecs.abc.Codec], key,
                                  array):
    ...
    def add_episode(
        ...
        for key, value in data.items():
            ...
            if key not in self.data:
                if is_zarr:
                    cks = self._resolve_array_chunks(chunks=chunks, key=key, array=value)
                    cpr = self._resolve_array_compressor(compressors=compressors,
                                                         key=key,
                                                         array=value)
                    arr = self.data.zeros(
                        name=key,
