# https://github.com/real-stanford/diffusion_policy/blob/main/diffusion_policy/common/timestamp_accumulator.py
from typing import List, Tuple, Optional, Dict
import math
import numpy as np


def get_accumulate_timestamp_idxs(timestamps: List[float],
                                  start_time: float,
                                  dt: float,
                                  eps: float = 1e-5,
                                  next_global_idx: Optional[int] = 0,
                                  allow_negative=False) -> Tuple[List[int], List[int], int]:
    """
...
def align_timestamps(timestamps: List[float],
